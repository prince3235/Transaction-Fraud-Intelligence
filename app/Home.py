...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown('<p style="font-size:11px;font-weight:700;color:#5A8AA8;text-transform:uppercase;letter-spacing:0.12em;margin-bottom:12px;">Quick Links</p>', unsafe_allow_html=True)
    if st.button("🚨 View Alerts", use_container_width=True):
        st.switch_page("pages/1_Alerts_Monitoring.py")
    if st.button("🧪 Sandbox Simulator", use_container_width=True):
        st.switch_page("pages/3_Transaction_Sandbox.py")
...
def sbadge(status):
    if status == "APPROVED":
        return f'<span class="status-badge status-approved">APPROVED</span>'
    elif status == "PENDING_REVIEW":
        return f'<span class="status-badge status-pending">PENDING</span>'
    elif status == "BLOCKED":
        return f'<span class="status-badge status-blocked">BLOCKED</span>'
    return f'<span class="status-badge status-pending">{status}</span>'

HEADS = ["ID","Timestamp","ML Prob","ML Risk","Final Risk","Score","Override", "Status"]
th = "".join(
...
for _, row in table_df.iterrows():
    ...
    status_str = row.get("status", "APPROVED")
    tbody += (
        f'<tr>'
